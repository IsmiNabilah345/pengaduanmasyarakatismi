...
<body>
    ...
    <form action="" method="post">
        <label>Cari :</label>
        <input type="text" name="keyword2" autocomplete="off">
    ...
    if (isset($_POST["cari"])) {
        $cari = $_POST["keyword2"];
        $role = cariPetugas($cari);
    }
    ?>
    <?php
    if (isset($_POST["semua"])) {
        $cariii = $_POST["keyword2"];
        $role = cariSemuaPetugas($cariii);
    }
    ?>
            ...
            <tbody>
                <?php foreach ($role as $row) : ?>
                    <tr>
                        ...
                        <td>
                            <a href="petugas_edit_ismi.php?id=<?= $row['id_petugas_ismi']; ?>" class="btn btn-dark btn-sm" title="Update">Edit</a>
                            <a href="petugas_hapus_ismi.php?id=<?= $row['id_petugas_ismi']; ?>" class="btn btn-dark btn-sm" oneclick="return confirm ('yakin?');"> Hapus</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
